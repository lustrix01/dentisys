...
assert_onboarding_contract(
    str_contains($facultyInvitation, 'faculty_invitation_name_parts_from_payload')
        && str_contains($facultyInvitation, 'pi.name_prefix AS canonical_name_prefix'),
    'Faculty invitation persists and returns canonical structured components'
);
assert_onboarding_contract(str_contains($facultyInvitation, "'prefix' => \$row['canonical_name_prefix']"), 'Faculty response keeps prefix mapped to the canonical prefix field');
