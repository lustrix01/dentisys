...
import { DEVELOPMENT_MOCK_STUDENT } from '../../services/developmentProviders';
...
export function SsoLogin() {
  ...
  const mockIdentityEnabled = runtimeConfig.providers.identity.development_mock_enabled;
  ...
  const handleGoogleSignIn = () => {
    if (!mockIdentityEnabled) {
      setError('Google sign-in is not configured in this environment. Use password login.');
      return;
    }
    setUser(DEVELOPMENT_MOCK_STUDENT);
    setAuthenticated();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (email.trim().toLowerCase() === 'student@bicol-u.edu.ph' && mockIdentityEnabled) {
      setUser(DEVELOPMENT_MOCK_STUDENT);
      setAuthenticated();
      setIsLoading(false);
      navigate('/student/dashboard', { replace: true });
    ...
    if (email.trim().toLowerCase() === 'student@bicol-u.edu.ph') {
      setError('Development mock Student identity is disabled. Use an existing password account.');
      setIsLoading(false);
      return;
    }
          ...
          <div className="md:col-span-7 p-6 sm:p-8 md:p-10 flex flex-col justify-center bg-white dark:bg-slate-900 transition-colors">
            <div className="max-w-md w-full mx-auto">
              ...
              <div className="mb-5">
                ...
                {mockIdentityEnabled && (
                  <div className="mb-4 rounded-xl border border-amber-300 bg-amber-50 px-3 py-2 text-[11px] font-semibold text-amber-800">
                    Development mock identity enabled. Student access is browser-only and cannot authorize backend data or attendance.
                  </div>
                )}
              ...
              >
                ...
                <span>{mockIdentityEnabled ? 'Development mock Student sign-in' : 'Google sign-in not configured'}</span>
              </button>
