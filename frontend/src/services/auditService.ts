...
let _auditIdentity: { name: string; role: string } | null = null;

export function setAuditIdentity(name: string, role: string): void {
  _auditIdentity = { name, role };
}
export function clearAuditIdentity(): void {
  _auditIdentity = null;
}

const currentUser = () => _auditIdentity;
...
export const recordAudit = (input: AuditInput) => { const user = currentUser(); if (!user) { return null; } const entry: AuditLog = { id: `audit-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`, timestamp: input.timestamp || new Date().toISOString(), userName: input.userName || user.name || 'System User', userRole: (input.userRole || user.role || 'faculty') as AuditRole, action: input.action, module: input.module, description: input.description, status: input.status, ipAddress: input.ipAddress || 'Frontend placeholder', device: input.device || navigator.userAgent || 'Browser placeholder' }; const logs = [entry, ...getAuditLogs()]; localStorage.setItem(key, JSON.stringify(logs.slice(0, 1000))); return entry; };
export const auditForCurrentUser = () => { const user = currentUser(); if (!user) { return []; } const logs = getAuditLogs(); return user.role === 'admin' ? logs : logs.filter(log => log.userName === user.name && log.userRole === user.role); };
