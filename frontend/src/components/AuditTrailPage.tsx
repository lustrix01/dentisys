...
export const AuditTrailPage: React.FC<Props> = ({ role, title, subtitle, allLogs = false, accent }) => {
  ...
  const fetchLogs = () => {
    ...
    if (role === 'admin') {
      ...
      getSecretaryActivityApi(100)
        .then((data) => {
          if (Array.isArray(data?.activity)) {
            const mapped: AuditLog[] = data.activity.map((a) => ({
              id: a.id,
              timestamp: a.timestamp,
              userName: a.userName || 'Class Secretary',
              userRole: (a.userRole as AuditRole) || 'secretary',
              action: a.action || 'Activity',
              module: a.module || 'secretary',
        ...
        .catch((err) => {
          setError(err instanceof Error ? err.message : 'Failed to retrieve secretary activity log.');
        })
        .finally(() => setLoading(false));
    } else {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-4">
        <div>
          <h1 className="text-2xl font-extrabold font-heading text-slate-800 dark:text-slate-100">{title}</h1>
          <p className="text-xs text-slate-400 mt-1">{subtitle}</p>
        </div>
        <div className="flex gap-2 no-print">
          <button onClick={exportCsv} className="flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-900">
            <Download className="w-4 h-4" />
            Export CSV
          </button>
          <button onClick={() => window.print()} className={`flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl text-white text-xs font-bold ${theme.button}`}>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 no-print">
        {cards.map(({ label, count, icon: Icon, iconClass, surface }) => (
          <Card key={label} className={`p-4 flex items-center justify-between border ${surface}`}>
            <div>
              <p className="text-[10px] uppercase tracking-wider font-bold text-slate-400">{label}</p>
              <p className="text-2xl font-extrabold text-slate-800 dark:text-slate-100 mt-1">{count}</p>
            </div>
            <div className={`p-2.5 rounded-xl bg-white/70 dark:bg-slate-900/60 ${iconClass}`}>
      ...
      <Card className="no-print">
        <CardContent className="p-4">
          <div className="flex flex-wrap gap-3 items-center">
            <Filter className={`w-4 h-4 ${theme.icon}`} />
            ...
            <button onClick={() => setSort(sort === 'newest' ? 'oldest' : 'newest')} className={`flex items-center gap-1.5 px-3 py-2 rounded-xl border text-xs font-bold ${theme.surface}`}>
      ...
      <Card className="p-0 overflow-hidden">
        <CardHeader className="border-b border-slate-100 dark:border-slate-800/80">
          <CardTitle className="flex items-center gap-2 text-sm">
            <ShieldCheck className={`w-4.5 h-4.5 ${theme.icon}`} />
            Activity records
          </CardTitle>
        </CardHeader>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
                {filtered.map(log => (
                  <tr key={log.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-900/30">
                    ...
                    <td className="px-5 py-3 text-slate-500">{log.module}</td>
                    <td className="px-5 py-3">
                      <span className={`px-2 py-0.5 rounded-md text-[9px] font-extrabold uppercase ${statusClass[log.status]}`}>
                        {log.status}
                      </span>
                    </td>
                    <td className="px-5 py-3 text-right">
                      <button onClick={() => setSelected(log)} className={`text-[10px] font-bold ${theme.icon}`}>
                        View details
                      </button>
                    </td>
                  </tr>
...
const Select = ({ value, onChange, options }: { value: string; onChange: (value: string) => void; options: string[] }) => (
  <select value={value} onChange={event => onChange(event.target.value)} className="px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-xs text-slate-600 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500">
    {options.map(option => (
