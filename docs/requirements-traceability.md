...
|---|---|---|---|---|
| SRC-CAP-SAD | Owner-provided DentiSys capstone/SAD paper | External owner source | Not found as a repository file | Requirements are represented from the approved execution prompt. |
...
| Requirement ID | Requirement description | Requirement source | Current repository evidence | Current status | Expected implementation phase | Expected validation or evidence artifact | Related decision ID | Open issue or Owner decision |
|---|---|---|---|---|---|---|---|---|
| ERD-001 | Users | SRC-CAP-SAD, SRC-FE-PROTO | Frontend auth service has mock users; backend has no users table | Prototype Only | Phase 1 | Phase 1 ERD approval artifact and later migration review | DEC-008, DEC-003 | Decide user identity model. |
| ERD-002 | Faculty | SRC-CAP-SAD, SRC-FE-PROTO | Faculty role and approval prototype exist | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-008, DEC-006 | Decide faculty profile and assignment structure. |
| ERD-003 | Students | SRC-CAP-SAD, SRC-FE-PROTO | Frontend `Student` type exists | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-008, DEC-009 | Decide student academic identity fields. |
| ERD-004 | Courses or subjects | SRC-CAP-SAD, SRC-FE-PROTO | `EnrolledSubject` and assessment subject codes exist in frontend | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-008, DEC-009 | Decide course, subject, term, and offering model. |
| ERD-005 | Sections | SRC-CAP-SAD, SRC-FE-PROTO | Class IDs/names exist in frontend types and workflows | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-009 | Decide section vs class offering boundaries. |
| ERD-006 | Enrollment | SRC-CAP-SAD, SRC-FE-PROTO | Enrolled subjects exist inside frontend student type | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-010 | Decide normalized enrollment records. |
| ERD-007 | Assessments | SRC-CAP-SAD, SRC-FE-PROTO | `Assessment` and `AssessmentScore` frontend types exist | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-011 | Decide assessment hierarchy and scoring model. |
| ERD-008 | Grades | SRC-CAP-SAD, SRC-FE-PROTO | Frontend grade computation exists; backend grades absent | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-011, DEC-012 | Decide computed vs stored grades and revisions. |
| ERD-009 | Retention | SRC-CAP-SAD, SRC-FE-PROTO | Retention status and logs exist in frontend type | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-013 | Decide policy versioning and history. |
| ERD-010 | Remedial records | SRC-CAP-SAD, SRC-FE-PROTO | General `RemedialExam` frontend type exists | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-014 | Add first, second, and cost-recovery decisions. |
| ERD-011 | Attendance | SRC-CAP-SAD, SRC-FE-PROTO | `AttendanceRecord` and override audit frontend types exist | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-015 | Decide attendance session and verification records. |
| ERD-012 | Devices | SRC-CAP-SAD, SRC-FE-PROTO | CCTV placeholder exists; no device model found | Owner Decision Required | Phase 1 | Phase 1 ERD approval artifact | DEC-015, DEC-019 | Decide camera/device registration needs. |
| ERD-013 | Facial templates | SRC-CAP-SAD, SRC-REPO-DOCS, SRC-FE-PROTO | Frontend stores simulated face images/details; docs mention encrypted vectors | Owner Decision Required | Phase 1 | Phase 1 ERD approval artifact and privacy review | DEC-016, DEC-017, DEC-018 | Decide template/image storage and retention. |
| ERD-014 | Audit logs | SRC-CAP-SAD, SRC-REPO-DOCS, SRC-FE-PROTO | Frontend localStorage audit exists; backend audit table absent | Prototype Only | Phase 1 | Phase 1 ERD approval artifact | DEC-007 | Decide append-only audit structure. |
