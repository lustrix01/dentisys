...
| ERD-014 | Audit logs | SRC-PLAINTEXT, SRC-CAP-SAD, SRC-ERD-1B, SRC-PHASE-1-RECON, SRC-REPO-DOCS, SRC-FE-PROTO | Phase 1B: `audit_log` table implemented (6 columns, migration 031). timestamp mapped to logged_at per B1-D12. FK to user_account. | Confirmed | Phase 1B (complete) | Phase 1B migration 031, stage manifest, validation evidence | DEC-007 | Append-only audit structure deferred to Phase 2. |
